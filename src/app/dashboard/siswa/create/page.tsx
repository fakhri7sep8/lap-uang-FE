...
const tambahSiswaSchema = Yup.object().shape({
  ...
  tipeProgram: Yup.string().required('Tipe program wajib diisi')
})
  ...
  const handleReadExcel = async (
    ...
    for (const i in ValueExcel?.json) {
      ...
      if (Object.values(r).every(v => v))
        arr.push({
          name: r?.Name,
          InductNumber: r?.No_induk,
          dorm: r?.Asrama,
          generation: r?.Generasi,
          major: r?.Jurusan,
          status: r?.Status,
          isDelete: false,
          NISN: r?.Nisn,
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      const payload = {
        ...
        tipeProgram: values.tipeProgram
      }
      mutate(payload)
      resetForm()
    }
      ...
      <form onSubmit={formik.handleSubmit} className='flex flex-col gap-6'>
        ...
        <div className='w-full h-full px-4 py-8 bg-white rounded-xl'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6 pb-4'>
            ...
            {renderSelect('Tipe Program', 'tipeProgram', [
              'FULLDAY',
              'BOARDING'
            ])}
