...
export default function VerifyTokenPage () {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('https://lap-uang-be.vercel.app/auth/verify-reset-token', {
        method: 'POST',
      ...
      if (data.resetSessionId)
        localStorage.setItem('resetSessionId', data.resetSessionId)
      ...
      <div className='w-full max-w-md bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200 p-8'>
        ...
        {searchParams.get('code') && (
          <div className='mb-4 p-3 bg-yellow-50 text-yellow-800 rounded-md text-sm'>
            Kode (dev):{' '}
            <span className='font-mono'>{searchParams.get('code')}</span>
          </div>
        )}
